...
<body>
    ...
    <div class="navbar">
        <h3>Admin Surat</h3>
        <div style="font-size: 14px; color:#666;">Tambah Data Penelitian</div>
    </div>
            ...
            <form action="{{ route('penelitian.store') }}" method="POST">
                ...
                <div class="form-grid">
                    ...
                    <div>
                        ...
                        <input type="text" name="univ" value="{{ old('univ') }}" required placeholder="Contoh: UNESA / ITS">
                        @error('univ') <p class="error">{{ $message }}</p> @enderror
                    </div>
                    <div class="full-width">
                        <label>Judul Penelitian / Skripsi</label>
                        <input type="text" name="judul" value="{{ old('judul') }}" required placeholder="Masukkan judul lengkap penelitian">
                        @error('judul') <p class="error">{{ $message }}</p> @enderror
                    </div>
                    <div>
                        <label>Tahun Penelitian</label>
                        <input type="number" name="tahun" value="{{ old('tahun') }}" required placeholder="Contoh: 2026">
