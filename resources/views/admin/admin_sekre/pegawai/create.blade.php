...
<div class="navbar"><h3>Admin Pegawai</h3><div>Tambah Data Pegawai</div></div>

<div class="container"><div class="card">
<div class="header"><h2>Tambah Data Pegawai</h2><a href="{{ route('pegawai.index') }}" class="back">← Kembali</a></div>
<form action="{{ route('pegawai.store') }}" method="POST">
@csrf
<div class="form-grid">
<div><label>Nama</label><input name="nama" value="{{ old('nama') }}" required>@error('nama')<div class="error">{{ $message }}</div>@enderror</div>
<div><label>NIP</label><input name="nip" value="{{ old('nip') }}" required>@error('nip')<div class="error">{{ $message }}</div>@enderror</div>
<div><label>Bidang</label><input name="bidang" value="{{ old('bidang') }}" required>@error('bidang')<div class="error">{{ $message }}</div>@enderror</div>
<div><label>Posisi</label><input name="posisi" value="{{ old('posisi') }}" required>@error('posisi')<div class="error">{{ $message }}</div>@enderror</div>
<div style="grid-column:1 / -1;"><label>Alamat</label><textarea name="alamat" required>{{ old('alamat') }}</textarea>@error('alamat')<div class="error">{{ $message }}</div>@enderror</div>
</div>
