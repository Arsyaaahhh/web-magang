...
<head>
  ...
  <style>
    * { margin: 0; padding: 0; box-sizing: border-box; font-family: 'Poppins', sans-serif; }
    ...
    @media (max-width: 600px) { .navbar { padding: 15px 20px; } .card { padding: 20px; } }
  </style>
</head>
<body>
  ...
  <div class="container">
    <div class="card">
      <div class="header">
        <h2>Tambah Rekap Alat Ukur</h2>
        <a href="/admin/admin_metro/alat" class="back">← Kembali</a>
      </div>

      <!-- Pastikan route store Anda menggunakan URL ini -->
      <form action="/admin/admin_metro/alat/store" method="POST">
        @csrf
        <div class="form-grid">
          <div>
            <label>Tahun</label>
            <!-- Menggunakan Tahun Sekarang dari Server (Opsional, tapi membantu admin) -->
            <input type="number" name="tahun" placeholder="Contoh: 2024" required min="2000">
          </div>
          <div>
            <label>Jumlah Potensi Alat Ukur Wajib Tera (Unit)</label>
            <input type="number" name="jumlah" placeholder="Masukkan Jumlah Keseluruhan" required min="0">
          </div>
        </div>
