...
<head>
  ...
  <style>
    * { margin: 0; padding: 0; box-sizing: border-box; font-family: 'Poppins', sans-serif; }
    ...
    /* 🔥 CSS diupdate agar select juga cantik */
    ...
    <div class="card">
      ...
      <form action="/admin/admin_metro/reparasi/update/{{ $data->id }}" method="POST">
        ...
        <div class="form-grid">
          <div>
            <label>Tahun</label>
            <input type="number" name="tahun" value="{{ old('tahun', $data->tahun) }}" required min="2000">
            ...
            <select name="kecamatan" required>
                ...
                @if(isset($list_kecamatan))
                    @foreach($list_kecamatan as $k)
                        <option value="{{ $k->NM_KECAMATAN }}" {{ old('kecamatan', $data->kecamatan) == $k->NM_KECAMATAN ? 'selected' : '' }}>
                            {{ $k->NM_KECAMATAN }}
                        </option>
                    @endforeach
                @endif
